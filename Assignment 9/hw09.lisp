...
(listp x) /\ (natp n) /\ ~(endp x) => (m1 (rest x) n) < (m1 x n)
         
1)
(defunc f1 (x n)
 :input-contract (and (listp x) (natp n) (not (endp x)))
...
3)
(listp x) /\ (natp n) /\ ~(endp x) => (m1 (rest x) n) = (len (rest x)) < (len x) = (m1 x n)
...
3)
(natp n) /\ (integerp i) /\ ~(i = 0) /\ (i < 0) => (m2 n (i + 1)) = (abs (i + 1)) < (abs i) = (m2 n i)

(natp n) /\ (integerp i) /\ ~(i = 0) /\ ~(i < 0) => (m2 (n + 1) (i - 1)) = (abs (i - 1)) < (abs i) = (m2 n i)
...
3)
(lonp l) /\ (natp n) /\ (> n 0)
                      => (m3 l (n - 1)) = ((len l) + (n - 1)) < ((len l) + n) = (m3 l n)
                      
(lonp l) /\ (natp n) /\ ~(> n 0)
                      => (m3 (rest l) n) = ((len (rest l)) + n) < ((len l) + n) = (m3 l n)
...
(implies (and (posp a) (lonp b) (posp c) (endp b))
         (< (m-omg a (cons c b) c) (m-omg a b c)))
        
(implies (and (posp a) (lonp b) (posp c) (not (endp b)) (< (first b) a))
         (< (m-omg a (cons (+ 1 (first b))(rest b)) c) (m-omg a b c)))
         
(implies (and (posp a) (lonp b) (posp c) (not (endp b)) (not(< (first b) a)) (not (equal (first b) a)))
         (< (m-omg a (cons a (rest b)) c) (m-omg a b c)))
...
(m-omg a (cons c b) c)
= {def of m-omg, C1, C2, C3}
(abs (- a (first (cons c b))))
= {first-rest axioms}
(abs (- a c))
< {Arithmetic}
(+ 1 (abs (- a c)))
...
1. ~(listp l) => phi-in-max
2. (listp 1) /\ (endp l) => phi-in-max
3. (listp l) /\ (not (endp l)) /\ phi-in-max|((l (rest l))) => phi-in-max
...
(implies (and (not (listp l)) (lonp l) (consp l))
         (in (max-l l) l))
...
(implies (and (listp l) (endp l) (lonp l) (consp l))
         (in (max-l l) l))
...
(implies (and (listp l) (not (endp l)) (lonp l) (consp l) (in (max-l (rest l)) (rest l)))
         (in (max-l l) l))

Two cases: (first l) < (max-l (rest l)), (first l) >= (max-l (rest l))         
...
C3. (lonp l)
C4. (consp l)
C5. (in (max-l (rest l)) (rest l))
C6. (first l) < (max-l (rest l))
--------------          

(in (max-l l) l)
= {def of max-l, C1, C3, C4, C6}
(in (max-l (rest l)) l)
= {def of in, C2}
(or (equal (max-l (rest l)) (first l)) 
...
2. (natp p) /\ (equal p 0) => phi_poweven
3. (natp p) /\ (not (equal p 0)) /\ phi_poweven|((p (- p 1))) => phi_poweven
...
1. (not (natp p)) => phi_poweven
(implies (and (not (natp p)) (rationalp x) (natp p))
         (>= (pow x (* p 2)) 0))
...
2. (natp p) /\ (equal p 0) => phi_poweven
(implies (and (natp p) (equal p 0) (rationalp x) (natp p))
         (>= (pow x (* p 2)) 0))
...
3. (natp p) /\ (not (equal p 0)) /\ phi_poweven|((p (- p 1))) => phi_poweven
(implies (and (natp p) (not (equal p 0)) (rationalp x) (natp p) (>= (pow x (* (- p 1) 2)) 0))
         (>= (pow x (* p 2)) 0))
...
= {Arithmetic}
(>= (pow x (+ (* (- p 1) 2) 2)) 0)
= {def of pow, C1, C3, C2, Arithmetic}
(>= (* x (pow x (+ (* (- p 1) 2) 1))) 0)
= {def of pow, C1, C3, C2, Arithmetic}
(>= (* x (* x (pow x (* (- p 1) 2)))) 0)
= {Arithmetic}
(>= (* (* x x) (pow x (* (- p 1) 2))) 0)
= {C3, T_esquare, C4, Arithmetic}
t
