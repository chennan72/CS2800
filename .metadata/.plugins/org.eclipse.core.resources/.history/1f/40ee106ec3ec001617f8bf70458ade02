...
Name of Group Members: Changzong Liu, Caden Shelman, Nihaal Korandla
...
(defconst *RL-rule* (RPSLK-rule 'R 'L "Rock crushes Lizarder"))
...
(defunc getWinner (p1 p2 rules)
  ...
  (cond ((or (equal p1 p2) (endp rules)) 0)
        ((and (equal p1 (RPSLK-rule-c1 (first rules))) (equal p2 (RPSLK-rule-c2 (first rules)))) 1)
        ((and (equal p2 (RPSLK-rule-c1 (first rules))) (equal p1 (RPSLK-rule-c2 (first rules)))) -1)
...
(defunc getWinMsg (p1 p2 rules)
  ...
  (cond ((endp rules) "No match")
        ((equal p1 p2) "Tie match")
        ((and (equal p1 (RPSLK-rule-c1 (first rules))) (equal p2 (RPSLK-rule-c2 (first rules)))) (RPSLK-rule-outcome (first rules)))
        ((and (equal p2 (RPSLK-rule-c1 (first rules))) (equal p1 (RPSLK-rule-c2 (first rules)))) (RPSLK-rule-outcome (first rules)))
...
(defunc runMatchesMsg (p1List p2List rules)
  ...
  (cond ((or (endp p1list) (endp p2list)) nil)
        (t (cons (genMatchMsg (first p1list) (first p2list) rules)
                 (runMatchesMsg (rest p1list) (rest p2list) rules)))))

(check= (runMatchesMsg *p1* *p3* *RPSLK-rules*)
        (LIST (LIST 'P2 "Rock crushes Scissors")
              (LIST 'P2 "Rock crushes Scissors")
              (LIST 'P2 "Rock crushes Scissors")
              (LIST 'P2 "Rock crushes Scissors")
              (LIST 'P2 "Rock crushes Scissors")))
(check= (runMatchesMsg *p2* *p3* *RPSLK-rules*)
        (LIST (LIST '- 'R)
              (LIST 'P1 "Paper covers Rock")
              (LIST 'P2 "Rock crushes Scissors")
              (LIST 'P2 "Rock crushes Lizarder")
              (LIST 'P1 "Spock vaporizes Rock")))
...
(check= (sortedp nil) t)
(check= (sortedp '(1)) t)
(check= (sortedp '(1 2 3 4 5 6 7)) t)
(check= (sortedp '(1 3 2)) nil)
...
(defunc isort (l)
  ...
  (cond ((or (endp l)(endp (rest l))) l)
        (t                            (insert (first l) (isort (rest l)))))) 

(check= (isort nil) nil)
(check= (isort '(1)) '(1))
(check= (isort '(5 6 7 4 2 3)) '(2 3 4 5 6 7))
...
(check= (ith-rational '(0 1 2 3 4 5) 4) 4)
(check= (ith-rational '(0 1 2 3 4 5) 0) 0)
(check= (ith-rational '(1) 0) 1)
...
(defunc median (l)
  ...
  :output-contract (rationalp (median l))
    (cond ((natp (/ (len l) 2)) (first (top-n-values l (nat/ (len l) 2))))
        (t               (first (top-n-values l (+ 1 (nat/ (len l) 2)))))))
...
(check= (median '(1)) 1)
(check= (median '(1 1 1 1 1 1 2 3 3 3 3 3 3)) 2)
...
;; DEFINE
(defdata grade-type (enum (list 'exam 'assignment 'quiz)))
...
(defdata quiz-grade (list 'quiz nat quiz-range))
(defdata assignment-grade (list 'assignment nat pct-range))
(defdata exam-grade (list 'exam nat pct-range))
                          
;; Union the 3 evaluation types.
(defdata grade-union (oneof quiz-grade assignment-grade exam-grade))
...
(defunc student-marks-list (l sn)
  ...
  (cond ((endp l) l)
        ((equal (grade-rec-id (first l)) sn) (cons (first l) (student-marks-list (rest l) sn)))
        (t (student-marks-list (rest l) sn))))

(check= (student-marks-list *cs2800* 55555555)
        (list (grade-rec 55555555 1 'Wahl '(assignment 4 75))
              (grade-rec 55555555 1 'Wahl '(quiz 2 12))
              (grade-rec 55555555 1 'Wahl '(quiz 1 24))))
(check= (student-marks-list *cs2800* 1000000)
        nil)
...
(defunc type-marks-list (l type)
  ...
  (cond ((endp l) l)
        ((equal (get-grade-type (first l)) type) (cons (first l) (type-marks-list (rest l) type)))
...
(defunc mean-top-n (l n)
  :input-contract (and (grade-spreadsheetp l) (posp n))
  :output-contract (rationalp (mean-top-n l n))
  (cond ((endp l) 0)
        (t (/ (sum-lor (top-n-values (extract-scores l) n)) n))))

(check= (mean-top-n *cs2800* 1) 85)
(check= (mean-top-n *cs2800* 2) 80)
(check= (mean-top-n *cs2800* 5) 214/5)
...
(defunc calc-grade-hlp (mean max pct)
  :input-contract (and (rationalp mean)
                       (rationalp pct)
                       (posp max))
  :output-contract (rationalp (calc-grade-hlp mean max pct))
  (/ (* mean pct) max))
...
(defunc calc-grade (l sn)
  ...
  (cond ((endp l) 0)
        (t (+ (calc-grade-hlp (mean-top-n (type-marks-list (student-marks-list l sn) 'quiz) 
                                          *num-quizzes*) *quiz-max* *pct-quizzes*)
              (+ (calc-grade-hlp (mean-top-n (type-marks-list (student-marks-list l sn) 'assignment) 
                                             *num-assignments*) 100 *pct-assignments*)
                 (calc-grade-hlp (mean-top-n (type-marks-list (student-marks-list l sn) 'exam) 
                                             *num-exams*) 100 *pct-exams*))))))
...
(defconst *cs2800-Test2* (list (grade-rec 1234567 2 'Sprague '(quiz 20 1))
                               (grade-rec 1234567 2 'Sprague '(quiz 19 1))
                               (grade-rec 1234567 2 'Sprague '(quiz 18 1))
                               (grade-rec 1234567 2 'Sprague '(quiz 17 1))
                               (grade-rec 1234567 2 'Sprague '(quiz 16 1))
                               (grade-rec 1234567 2 'Sprague '(quiz 15 1))
                               (grade-rec 1234567 2 'Sprague '(quiz 14 1))
                               (grade-rec 1234567 2 'Sprague '(quiz 13 1))
                               (grade-rec 1234567 2 'Sprague '(quiz 12 1))
                               (grade-rec 1234567 2 'Sprague '(quiz 11 1))
                               (grade-rec 1234567 2 'Sprague '(quiz 10 1))
                               (grade-rec 1234567 2 'Sprague '(quiz 9 1))
                               (grade-rec 1234567 2 'Sprague '(quiz 8 1))
                               (grade-rec 1234567 2 'Sprague '(quiz 7 1))
                               (grade-rec 1234567 2 'Sprague '(quiz 6 1))
                               (grade-rec 1234567 2 'Sprague '(quiz 5 1))
                               (grade-rec 1234567 2 'Sprague '(quiz 4 1))
                               (grade-rec 1234567 2 'Sprague '(quiz 3 1))
                               (grade-rec 1234567 2 'Sprague '(quiz 2 1))
                               (grade-rec 1234567 2 'Sprague '(quiz 1 1))
                               (grade-rec 1234567 2 'Sprague '(assignment 11 100))
                               (grade-rec 1234567 2 'Sprague '(assignment 10 100))
                               (grade-rec 1234567 2 'Sprague '(assignment 9 100))
                               (grade-rec 1234567 2 'Sprague '(assignment 8 100))
                               (grade-rec 1234567 2 'Sprague '(assignment 7 100))
                               (grade-rec 1234567 2 'Sprague '(assignment 6 100))
                               (grade-rec 1234567 2 'Sprague '(assignment 5 100))
                               (grade-rec 1234567 2 'Sprague '(assignment 4 100))
                               (grade-rec 1234567 2 'Sprague '(assignment 3 100))
                               (grade-rec 1234567 2 'Sprague '(assignment 2 100))
                               (grade-rec 1234567 2 'Sprague '(assignment 1 100))
                               (grade-rec 1234567 2 'Sprague '(exam 2 100))
                               (grade-rec 1234567 2 'Sprague '(exam 2 100))))
...
(check= (calc-grade *cs2800-Test2* 1234567) 
                (+ (* 20 1/24) (+ (* 20 100/100) (* 60 100/100))))#|ACL2s-ToDo-Line|#
