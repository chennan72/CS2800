...
(defdata RPSLK-rule (record (win . RPSLK) ;; SOLUTION
                            (lose . RPSLK)
                            (message . string)))
...
(defdata rule-list (listof RPSLK-rule)) ; SOLUTION
(defconst *RPSLK-rules* ; SOLUTION
...
(defunc getWinner (p1 p2 rules)
  ...
  (if (endp rules)
    0
    (let* ((win (RPSLK-rule-win (first rules)))
           (lose (RPSLK-rule-lose (first rules))))
      (cond ((and (equal p1 win)(equal p2 lose)) 1)
            ((and (equal p2 win)(equal p1 lose)) -1)
            (t (getWinner p1 p2 (rest rules)))))))
; Solution Note: I wanted to use a let and cond to show
; how they could be used, even within an if statement.
; Your solution does not need to look like this. Other
; approaches could be 1) helper functions to get a winner
; and loser 2) Use a bunch of if statements (bit ugly)
; and 3) don't use let (repeat function calls for no reason)
...
(defconst *p4* (list 'P 'P 'P 'P 'P))#|ACL2s-ToDo-Line|#
...
(defunc getWinMsg (p1 p2 rules)
  ...
  (if (endp rules)
    ...
    (let* ((win (RPSLK-rule-win (first rules)))
           (lose (RPSLK-rule-lose (first rules)))
           (msg (RPSLK-rule-message (first rules))))
      (cond ((and (equal p1 win)(equal p2 lose)) msg)
...
;; SOLUTION FOR MARKERS: Be very open minded about this.  A more generic
;; rule list should ideally be created (and the current
;; rule-list would simply be one type of rule list using RPSLK 
;; signs). From there the functions above wouldn't have to change
;; except RPSLK signs would have to be replaced with something
;; more generic.  This isn't many points so I want you to
;; reward creativity with full points even if the code looks
;; sub-optimal.  This is meant to be a fun exercise.
...
(defunc sortedp (l)
  :input-contract (lorp l) ; (listp l)
  :output-contract (booleanp (sortedp l))
  (cond ((or (endp l)(endp (rest l)))  t) ;((endp l) t)
...
(test? (implies (lorp l) (sortedp (isort l))))
...
;; Solution NOTE:(input for i) and ensure that i < (len l)
(defunc ith-rational (l i)
  :input-contract (and (lorp l)(natp i)(< i (len l)));(integerp i))
...
(defunc median (l)
  ...
  (let* ((lsorted (isort l))
        (lenHalfL (nat/ (len lsorted) 2)))
    (if (< lenHalfL (len lsorted))
      (ith-rational lsorted  lenHalfL)
      0)))
; Note: this extra condition was needed due to the Input-contract
; I gave to ith-rational. ACL2s needs to know lenHalfL is smaller
; than the length of the sorted list.
...
;; First define an enumerated datatype for evaluation type (exam, assignment or quiz).
;; SOLUTION
...
(defdata quiz-range (range rational(0 <= _ <= *quiz-max*)))

;; Define information for the 3 different evalution types
;; with each type having a type, number (positive number) and grade.
(defdata quiz-info (list 'quiz pos quiz-range))
(defdata exam-info (list 'exam pos pct-range))
(defdata assignment-info (list 'assignment pos pct-range))
;; Union the 3 evaluation types.
(defdata grade-info (oneof exam-info quiz-info assignment-info)) ; ...
...
(check= (grade-recp (first (list (grade-rec 1234567 2 'Sprague '(quiz 5 12))
                                 (grade-rec 1234567 2 'Sprague '(assignment 2 88))
...
(defunc type-marks-list (l type)
  ...
  (cond ((endp l) nil)
        ((equal (get-grade-type (first l)) type) 
...
(defunc mean-top-n (l n)
  ...
  (/ (sum-lor (top-n-values (extract-scores l) n)) n))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; calc-grade-hlp: Rational x Rational x Pos -> Rational
;; (calc-grade-hlp mean pct max) is a helper function for calc-grade
...
(defunc calc-grade (l sn)
  ...
  (let* ((student-records (student-marks-list l sn))
         (quiz-records (type-marks-list student-records 'quiz))
         (exam-records (type-marks-list student-records 'exam))
         (assignment-records (type-marks-list student-records 'assignment)))
    (+ (calc-grade-hlp (mean-top-n quiz-records *num-quizzes*) 
                       *pct-quizzes* *quiz-max*)
       (+ (calc-grade-hlp (mean-top-n assignment-records *num-assignments*) 
                          *pct-assignments* 100)
          (calc-grade-hlp (mean-top-n exam-records *num-exams*) 
                          *pct-exams* 100)))))
