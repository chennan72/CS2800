...
1. ~(rationalp e)/\(rationalp r)/\(lorp l)/\(in e (filter-less r l)) => (< e r)
2. (rationalp e)/\(rationalp r)/\(lorp l)/\(in e (filter-less r l)) /\ (endp (filter-less r l)) => (< e r)
3. (rationalp e)/\(rationalp r)/\(lorp l)/\(in e (filter-less r l)) /\ ~(endp (filter-less r l)) 
   /\ (equal e (first (filter-less r l))) => (< e r)
4. (rationalp e)/\(rationalp r)/\(lorp l)/\(in e (filter-less r l)) /\ ~(endp (filter-less r l)) 
   /\ ~(equal e (first (filter-less r l))) /\ (in e (rest (filter-less r l))) => (< e r)
   
1: ~t => (< e r) = t


2: (in e (filter-less r l)) /\ (endp (filter-less r l)) = nil {def in}
   nil => (< e r) = t
...
C5. (not (endp (filter-less r l)))
C6. (equal e (first (filter-less r l)))
-------------------------
C7. (first (filter-less r l)) < r  {def filter-less, C2, C3, C5}
C8. e < r  {C6, C7}
...
C4. (in e (filter-less r l))
C5. (not (endp (filter-less r l)))
C6. (not (equal e (first (filter-less r l))))
C7. (in e (rest (filter-less r l)))
...
C7. (in e l)  {C4, C6}
C8. e = (first l) {C7, C5}
C9. e = (first (filter-less r l))  {C4, C6, C8}

(< e r)
= {C9}
(< (first (filter-less r l)) r)
= {def filter-less}
t
...
C4. (in e (filter-less r l))
C5. (not (endp (rest l)))
C6. (rationalp e)/\(rationalp r)/\(lorp (rest l))/\(in e (filter-less r (rest l))) => (< e r)
----------------------
C7. (consp (filter-less r l))  {C4, def of in}
C8. (consp l)    {C6, def of filter-less}
C9. (lorp (rest l)) {C3, C8}
C10. (in e (filter-less r (rest l))) => (< e r) {C1, C2, C9}
C11. (or (equal e (first (filter-less r l))) (in e (rest (filter-less r l))))  {C4, def in}
...
Induction Scheme: sortedp

1. ~(lorp l) => phi_qsort
2. (lorp l) /\ (or (endp l) (endp (rest l))) => phi_qsort
3. (lorp l) /\ (not (or (endp l) (endp (rest l)))) /\
   phi_qsort|((l (rest l))) => phi_qsort
...
C3. (sortedp (qsort (rest l)))
-------------------
(sortedp (qsort l))
= {def sortedp, C1, C2}
(and (<= (first (qsort l)) (second (qsort l))) (sortedp (rest (qsort l))))
...
C1. (lorp l)
C2. ~(lorp l)
C3. (or (endp l) (endp (rest l)))
C4. (lorp l) /\ (not (or (endp l) (endp (rest l)))) /\
    phi_qsort|((l (rest l))) => phi_qsort
...
(defunc gen-inc (n x)
  :input-contract (and (rationalp x) (natp n)) 
  :output-contract (listp (gen-inc n x))
  (if (equal n 0)
    '()
    (cons x (gen-inc (- n 1) (+ 1 x))))) 

(check= (gen-inc 0 1) '())
(check= (gen-inc 1 1) '(1))
(check= (gen-inc 5 1) '(1 2 3 4 5))

(defconst *slow-list* (gen-inc 16000 1))

(acl2::er-progn
   (acl2::time$ (acl2::value-triple (ssort *slow-list*)))
   (acl2::value-triple nil))
;; How long does this take?
; 8.77 seconds realtime, 8.75 seconds runtime

(acl2::er-progn
   (acl2::time$ (acl2::value-triple (qsort *slow-list*)))
   (acl2::value-triple nil))
;; How long does this take?
; 11.87 seconds realtime, 11.79 seconds runtime
